...
### Пороги Dumb Zone в Cline

Cline резервирует буфер под конец контекста:

| Модель | Контекст | Буфер | Эффективный лимит |
|--------|----------|-------|-------------------|
| DeepSeek | 64K | 27K | ~37K |
| Большинство моделей | 128K | 30K | ~98K |
| Claude | 200K | 40K | ~160K |

Попадание в Dumb Zone — не вопрос «если», а вопрос «когда». Чем больше контекст, тем быстрее середина перестаёт работать.

### Управление контекстом

```
[0%──────────────────────────────80%──────────────100%]
  Нормальная работа               │  Dumb Zone
                                  │
                                  ▼
                           /compact (VS Code)
                           /compact (CLI TUI)
                           --compaction agentic (headless)
```
